...
import { fetchPlansAPI } from "../../services/planAPI";
...
const Pricing = () => {
  ...
  const freePlan = data?.plans?.filter((item) => item.planName === "Free");
  const premiumPlan = data?.plans?.filter(
    (item) => item.planName === "Premium"
  );

  if (isSuccess) {
    console.log("free plan", freePlan.length);
  }
          ...
          <div className="w-full lg:w-1/2 p-4">
            <div className="bg-orange-500 rounded-3xl px-8 lg:px-10 pb-14 pt-10 h-full">
              ...
              <Link
                ...
                to={`/dashboard/premium-subscription/${premiumPlan?.[0]?._id}`}
              >
