...
const AccountSummaryDashboard = () => {
  ...
  const hasEmail = data?.userFound?.email;

  //check if user has plan

  const hasPlan = data?.userFound?.hasSelectedPlan;

  //check if user has verified account
  const isEmailVerified = data?.userFound?.isEmailVerified;

  //total followers
  const totalFollowers = data?.userFound?.followers?.length;

  //total following
  const totalFollowing = data?.userFound?.followings?.length;

  //get user posts

  const userPosts = data?.userFound?.posts?.length;
